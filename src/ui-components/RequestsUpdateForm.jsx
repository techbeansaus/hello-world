...
export default function RequestsUpdateForm(props) {
  const {
    id: idProp,
    requests: requestsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = requestsRecord
      ? { ...initialValues, ...requestsRecord }
      : initialValues;
    setName(cleanValues.name);
    setPhoneNumber(cleanValues.phoneNumber);
    setEmailAddress(cleanValues.emailAddress);
    setRequestType(cleanValues.requestType);
    setRequestMessage(cleanValues.requestMessage);
    setErrors({});
  };
  const [requestsRecord, setRequestsRecord] = React.useState(requestsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Requests, idProp)
        : requestsModelProp;
      setRequestsRecord(record);
    };
    queryData();
  }, [idProp, requestsModelProp]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Requests.copyOf(requestsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "RequestsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || requestsModelProp)}
