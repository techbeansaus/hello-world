...
export default function RequestTypeUpdateForm(props) {
  const {
    id: idProp,
    requestType: requestTypeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = requestTypeRecord
      ? { ...initialValues, ...requestTypeRecord }
      : initialValues;
    setRequestType(cleanValues.requestType);
    setRouteEmail(cleanValues.routeEmail);
    setErrors({});
  };
  const [requestTypeRecord, setRequestTypeRecord] =
    React.useState(requestTypeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(RequestType, idProp)
        : requestTypeModelProp;
      setRequestTypeRecord(record);
    };
    queryData();
  }, [idProp, requestTypeModelProp]);
  React.useEffect(resetStateValues, [requestTypeRecord]);
  const validations = {
    requestType: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            RequestType.copyOf(requestTypeRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "RequestTypeUpdateForm")}
      {...rest}
    >
      <TextField
        label="Request type"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || requestTypeModelProp)}
